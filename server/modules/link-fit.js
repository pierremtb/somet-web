const linkFIT = (path, id) => {
  _convertToTcx(path, id);
  _updateWorkout(id, _parseTcx());
};

let _convertToTcx = (path, id) => {
...
let _updateWorkout = (id, data) => {
  WorkoutsDB.update(id, {$set : {fit_linked: true, fit_values: data}});
};

Modules.server.linkFIT = linkFIT;
